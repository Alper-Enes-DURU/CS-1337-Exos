...
Can use initialization list to initialize a structure variable
when it's defined.
Partial list is possible. Omitted values are set to zero, or empty string.

Arrays of structures can be used instead of
*/
...
int main()
{
    struct Student // Define a structure, no memory is allocated
    {
        string netID;
        double GPA;
    };

    // Define a structure variable, memory is allocated
    Student stu1;
    ...
    cout << endl << "Initialization with partial list" << endl;
