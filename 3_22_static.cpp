...
/*
    A static member variable is associated with a class, and not
...
and defined (memory is allocated) outside of the class declaration.
    A simple rule is to put the definition in the className.cpp, if 
it exists.
    A static member function allows us to access a static member variable
even if no object exists. It is called from the class, e.g. Tree::getTreeCountStatic()
    It can also be called from an object, e.g. t1.getTreeCountStatic()

    A static member function is not allowed to access an instance member variable
...
class Tree
{
    private:
        ...
        int type;
        static string descr[3]; // static array
    public:
        Tree(int type_)
        {
            type = type_;
...
int Tree::treeCount = 0; // Static member variable definition
...
int main()
{
    ...
    // Another use of static variables
