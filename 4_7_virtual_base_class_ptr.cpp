...
function is redefined (example of printObject)
    ...
    With dyynamic binding, the mapping is done not at compile time,
but at run time, when the object type (base or derived) is known

    To enable dynamic binding, declare the function "virtual" and
pass the object by reference or pass a pointer to the object
    Another concept in OOP: Polymorphism ("multiple shapes" in Greek)
    A function can take multiple shapes (or multiple versions) depending
on the type (base or derived) that calls the function.
    Once a function is declared virtual, all the versions in the derived
classes (child, grandchild, etc.) are automatically virtual.
You can omit the "virtual" keyword, but it's a good practice
to explicitly declare virtual.

    A virtual function is overridden, a non-virtual function
is redefined.

    Base class pointer: A pointer to a base class object can hold
the address of a derived class object.

    Through the base class pointer, you can access only the members
defined in the base class, even if thepointer points to a derived object.

    When called from a case class pointer, the correct version will
...
class UGStudent : public Student
{
    public:
        ...
        virtual double calcGPA() const // Redefine the calcGPA function
        {
            double GPA;
            if (avgScore > 98)
                GPA = 4.4;
